# qwen2_PE.py (at the very top, before any bitsandbytes imports)
import os
# disable Triton kernels in bitsandbytes so it falls back to pure-Python/CUDA
os.environ["BITSANDBYTES_DISABLE_TRITON"] = "1"
os.environ["BITSANDBYTES_FORCE_CPU"] = "1"
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    if intervention is not None:
        start_idx = intervention['question_start']
        end_idx = intervention['question_end']
        if module.layer_idx in intervention['layer_indices']:
            if intervention['type'] == 'ours':
                attn_weights[:,:,:, start_idx : end_idx] = attn_weights[:,:,:, start_idx : end_idx] + intervention['relevance'] * intervention['alpha']
            elif intervention['type'] == 'attention_replace':
                # (H, q_len)  ← one matrix per head
                rel = intervention["relevance"][
                    intervention["layer_indices"].index(module.layer_idx)]                                     # shape (H, q_len)

                # add batch-axis (dim-0) and query-axis (dim-2)
                rel = rel.unsqueeze(0).unsqueeze(2)   # → (1, H, 1, q_len)

                # make sure dtype / device match the attention tensor
                rel = rel.to(attn_weights)

                # now the assignment works — rel is broadcast to (B, H, T, q_len)
                attn_weights[:, :, :, start_idx:end_idx] = attn_weights[:, :, :, start_idx:end_idx] + rel * intervention['alpha']
            else:
                raise ValueError(f"Unknown intervention type: {intervention['type']}. Supported types are 'ours' and 'attention_replace'.")
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # output_hidden_states: Optional[bool] = False, # change
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # change
        # attention_states = hidden_states
